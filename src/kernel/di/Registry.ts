...
export class Registry {
  ...
  register(impl: Constructor) {
    ...
    if (this.providers.has(token)) {
      throw new Error(`Provider with token ${token} is already registered in the registry.`);
    }
  ...
  resolve<TImpl extends Constructor>(impl: TImpl): InstanceType<TImpl> {
    ...
    if (!provider) {
      throw new Error(`Provider with token ${token} is not registered in the registry.`);
    }
